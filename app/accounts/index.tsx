...
export default function AccountsScreen() {
  ...
  const visible = useMemo(() => {
    ...
    if (tab === 'bank') return accounts.filter((item) => item.type === 'bank' || item.type === 'other');
    if (tab === 'cash') return accounts.filter((item) => item.type === 'cash' || item.type === 'wallet');
      ...
      <PageScroll style={styles.scroller} contentContainerStyle={styles.content}>
        ...
        {accounts.length === 0 ? (
          <EmptyState
            ...
            message="Add a bank account, cash wallet, or credit card. Existing transactions stay available until you assign them."
          ...
          <Card elevated={false} style={[styles.list, isDesktop && styles.listDesktop]}>
...
const styles = StyleSheet.create({
  scroller: { flex: 1, minHeight: 0, width: '100%' },
  content: { padding: spacing.lg, gap: spacing.md, width: '100%', maxWidth: '100%' },
  list: { paddingVertical: 4, paddingHorizontal: 8 },
  listDesktop: { alignSelf: 'stretch' },

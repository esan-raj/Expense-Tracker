...
export function BudgetCard({ item, onPress }: BudgetCardProps) {
  ...
  const tone = item.percent >= 100 ? 'danger' : item.percent >= 75 ? 'warning' : 'success';
  const status =
    item.percent >= 100 ? 'Exceeded' : item.percent >= 90 ? 'Approaching limit' : item.percent >= 75 ? 'Approaching limit' : 'Healthy';

  return (
    <Pressable
      onPress={onPress}
      accessibilityRole="button"
      accessibilityLabel={`${item.categoryName ?? 'Overall budget'}, ${status}, ${item.percent} percent`}
